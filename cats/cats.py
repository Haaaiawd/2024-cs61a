...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def subjects_contains(x, subject):
        x = remove_punctuation(x.lower())
        ##得去掉标点符号
        x_list = x.split()
        subject = [i.lower() for i in subject]
        for i in subject:
            if i in x_list:
                return True
        return False
    return lambda x: subjects_contains(x, subject)
###用lambda,不然x没有定义。
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if typed_words !=[] and source_words!=[]:
        for i in range(max(len(source_words),len(typed_words))):
            if typed_words[i] == source_words[i]:
                k += 1
            if i >= min(len(typed_words),len(source_words)) - 1:
                break
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        if min(differences) > limit:
            return typed_word
        min_diff = differences.index(min(differences))
        return word_list[min_diff]
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def helper(typed, source, limit, times=0, score=0):
        if score > limit:
            return limit + 1
        if times + 1 > min(len(typed), len(source)):
            return max(len(typed), len(source)) - min(len(typed), len(source))
        else:  
            if typed[times] == source[times]:
                return 0 + helper(typed, source, limit, times + 1, score)
            else:
                return 1 + helper(typed, source, limit, times + 1, score + 1)
    return helper(typed, source, limit, times=0, score=0)
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[] for _ in range(len(tpp))]  # You may remove this line
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        min_time = min(get_time(times, k, i) for k in player_indices)
        for j in player_indices:
            if get_time(times, j, i) == min_time:
                fastest_players[j].append(words[i])
                break
